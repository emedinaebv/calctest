      ...
      chartInstance.data.datasets.forEach(function (dataset, i) {
        ...
        if (!meta.hidden) {
          meta.data.forEach(function (element, index) {
            ...
            if(element._model.backgroundColor == "#a3cf7a"){
              font = 11
            }
            ...
            if (data !== 0 ) {
              ...
              let dataString = Number.isInteger(data) ? data.toString() : data.toFixed(1);
...
axios.get("./assets/lang/main.json").then((lang) => {
    axios.get("./dbnew.json?v=24711_0012").then((db) => {
    axios.get("./colors.json").then((colorPattern) => {
      ...
      axios.get('./inform.json').then(inform => {
        ...
        Vue.component('bi-content', {
            ...
            methods: {  
            dlCanvas: function () {
                // Verifica que `chart_id` está definido
                if (!this.chart_id) {
                    console.error("chart_id no está definido");
                    return;
                }
            
                // Extraer el número del `chart_id` y sumarle 4
                let baseId = this.chart_id.replace("bi-content-chart_", ""); // Obtiene solo el número
                let newId = "bi-content-chart_" + (parseInt(baseId, 10) + 3); // Suma 4 al número
            
                var canvas = document.getElementById(newId);
            ...
            dlCanvas2: function () {
                ...
                let newId = "bi-content-chart_" + (parseInt(baseId, 10) + 3); 
